...
## Установка общеполезных пакетов

Для удобного подключения стразу переходим на TmuX.
...
# добавление репозитория (использует кодовое имя системы, для 24.04 это noble) sudo add-apt-repository "deb https://cloud.r-project.org/bin/linux/ubuntu $(lsb_release -cs)-cran40/" 
sudo apt update -qq
```

### Системные библиотеки для R и R-пакетов
```
sudo apt install -y --no-install-recommends r-base build-essential gfortran libcurl4-openssl-dev libssl-dev libxml2-dev libpng-dev libjpeg-dev libbz2-dev liblzma-dev libblas-dev liblapack-dev libreadline-dev libpcre2-dev libgmp-dev libmpfr-dev libcairo2-dev libxt-dev libx11-dev libfreetype6-dev libfontconfig1-dev libharfbuzz-dev libfribidi-dev libudunits2-dev libgdal-dev libgeos-dev libproj-dev libv8-dev libmagick++-dev libpq-dev unixodbc-dev fonts-dejavu fonts-roboto mesa-common-dev libgl1-mesa-dev libglu1-mesa-dev xorg xvfb qpdf jq imagemagick chrony postgresql
```

### LaTeX (опционально, для PDF через R Markdown)

```
sudo apt install -y texlive-full
```

### Python 3 и dev-окружение

```
# базовый Python 3 и инструменты разработки 
sudo apt install -y   python3   python3-pip   python3-venv   python3-dev   python3-openssl
```

Если планируется сборка собственных версий Python (pyenv и т.п.):
...
```
sudo apt update -qq 
sudo apt install -y --no-install-recommends r-base libcurl4-openssl-dev libssl-dev libxml2-dev libc6 libclang-dev libstdc++6 psmisc sudo gdebi-core
```

### Пользователь под RStudio

Создать обычного пользователя, под которым будут логиниться в веб‑интерфейс (не root):
```
...
## Установка RStudio Server
# Установка R на Ubuntu 24.x LTS
